...
>(({ className, ...props }, ref) => (
	<AccordionPrimitive.Item
		ref={ref}
		className={cn("rounded-2xl border border-[#CACACA] p-4", className)}
		{...props}
	/>
))
...
>(({ className, children, ...props }, ref) => (
	<AccordionPrimitive.Header className='flex'>
		<AccordionPrimitive.Trigger
			ref={ref}
			className={cn(
				"flex flex-1 items-start justify-between font-medium transition-all [&[data-state=open]>svg]:rotate-45",
				className,
			)}
			{...props}
		>
